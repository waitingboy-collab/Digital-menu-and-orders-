...
async function loadSalesStats() {
    ...
    if (top5El) {
        ...
        if (top5.length === 0) {
            ...
            top5El.innerHTML = top5.map(([name, stats], i) => `
                <div class="bg-amber-50 border border-amber-200 rounded-xl p-3 text-center">
                    <div class="text-lg">${medals[i]}</div>
                    <p class="text-xs font-bold text-slate-800 truncate" title="${name}">${name}</p>
                    <p class="text-[11px] text-amber-700 font-semibold">${stats.qty} бр.</p>
                </div>
            `).join('');
        }
    }
